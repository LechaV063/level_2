...
void go(struct snake_t *head)
{
    ...
    case LEFT:
        ...
        break;
    case RIGHT:
        if (head->x >= max_x) // Циклическое движение, чтобы не
                              // уходить за пределы экрана
            head->x = 0;
        mvprintw(head->y, ++(head->x), "%c", ch);
        break;
    case UP:
        if (head->y <= MIN_Y) // Циклическое движение, чтобы не
                              // уходить за пределы экрана
            head->y = max_y;
        mvprintw(--(head->y), head->x, "%c", ch);
        break;
    case DOWN:
        if (head->y >= max_y) // Циклическое движение, чтобы не
                              // уходить за пределы экрана
            head->y = MIN_Y;
        mvprintw(++(head->y), head->x, "%c", ch);
        break;
...
int checkDirection(snake_t *snake, int32_t key)
{
    ...
    for (size_t i = 0; i < CONTROLS; i++)
    {
        ...
        if (
            (snake->direction == UP && key == snake->controls[i].down) ||
            (snake->direction == DOWN && key == snake->controls[i].up) ||
            (snake->direction == LEFT && key == snake->controls[i].right) ||
            (snake->direction == RIGHT && key == snake->controls[i].left))
        {
            result = 0;
        }
        /* code */
